...
for try in $list; do
	((idx++))
	echo "file_path, file_no, file_name, class, qp, resolution, scale, mAP" >> ${1}_try${idx}_results.csv
			...
			qp=$(echo ${f} | cut -d '_' -f7)
       		...
       		echo \
				...
				${fdx}, \
       			...
       			${res}, \
       			${scale}, \
      			$(head -n 1 ${file} | cut -d '=' -f2)" >> ${1}_try${idx}_results.csv
	done 
done
